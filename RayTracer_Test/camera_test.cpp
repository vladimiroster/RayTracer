...
TEST(TestCamera, TestCameraCreate) {
  Camera c(std::make_pair(160, 120), static_cast<float>(M_PI_2));

  EXPECT_EQ(160, c.hsize());
  EXPECT_EQ(120, c.vsize());
  EXPECT_TRUE(ApproxEqual(static_cast<float>(M_PI_2), c.fov(), epsilon));
  EXPECT_EQ(Transform::id(), c.view());
}

TEST(TestCamera, TestPixelSizeHorizCanvas) {
  Camera c(std::make_pair(200, 125), static_cast<float>(M_PI_2));
  ASSERT_TRUE(ApproxEqual(0.01f, c.pixel_size(), epsilon));
}

TEST(TestCamera, TestPixelSizeVertCanvas) {
  Camera c(std::make_pair(125, 200), static_cast<float>(M_PI_2));
  ASSERT_TRUE(ApproxEqual(0.01f, c.pixel_size(), epsilon));
}

TEST(TestCamera, TestRayThroughCenter) {
  Camera c(std::make_pair(201, 101), static_cast<float>(M_PI_2));
...
TEST(TestCamera, TestRayCameraTransformed) {
  Camera c(std::make_pair(201, 101), static_cast<float>(M_PI_2), Transform::id().translate(0, -2, 5).rot_y(static_cast<float>(M_PI_4)));
  auto r = c.ray_for_pixel(100, 50);
...
TEST(TestCamera, TestRender) {
  ...
  Vector up(0, 1, 0);
  Camera c(std::make_pair(11, 11), static_cast<float>(M_PI_2), Transform::id().view(from, to, up));
